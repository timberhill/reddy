...
class Plot(object):
    ...
    def _colour(accent=False):
        ...
        if accent:
            ...
            i = Plot.colour_index
            Plot.colour_index = Plot.colour_index + \
                1 if Plot.colour_index < len(colours)-1 else 0
            return colours[i]
    ...
    def timeseries_yearly(times, values, years, bin_where="post", accent=False):
        ...
        if len(times) != len(values):
            raise ValueError(
                f"times [len={len(times)}] and values [len={len(values)}] must have the same length.")
        ...
        plt.style.use(os.path.join(os.path.dirname(__file__),
                                   "mpstyles/reddy-timeseries.mplstyle"))
